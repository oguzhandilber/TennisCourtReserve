...
def get_courts():
    ...
    if search_term:
        ...
        # Searching in court name and address
        search_filter = Court.name.ilike(f"%{search_term}%")
        if hasattr(Court, 'address'): # Check if address attribute exists
             search_filter = or_(search_filter, Court.address.ilike(f"%{search_term}%"))
