...
# Assuming create_notification is in notifications.py or a shared utility
from src.routes.notifications import create_notification as create_notification_util
import datetime
...
def approve_booking(booking_id):
    ...
    create_notification_util(
        ...
        content=f"Your booking for {booking.court.name} on {booking.start_time.strftime('%b %d at %H:%M')} has been approved by the court responsible.",
...
def decline_booking(booking_id):
    ...
    # Notify the user who made the booking, including reason if provided
    create_notification_util(
        user_id=booking.user_id,
        notification_type="booking_status",
        content=f"Your booking for {booking.court.name} on {booking.start_time.strftime('%b %d at %H:%M')} has been declined by the court responsible. Reason: {reason}",
        related_id=booking.id
    )
